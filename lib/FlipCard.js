...
export default class FlipCard extends Component {
  ...
  render() {
    ...
    if (this.props.flipVertical) {
      transform.push({
        skewX: this.state.rotate.interpolate({
          inputRange: [0, 1],
          outputRange: ['0deg', '180deg']
        })
      });
          ...
          onLayout={event => {
            ...
            if (this.state.mesured) {
              if (this.props.alignHeight) {
                this.setState({
                  height: Math.max(this.state.face.height, this.state.back.height)
                });
              }
...
export class Back extends Component {
  render() {
    ...
    return (
      <View style={[S.back, this.props.style, { transform: transform }]} onLayout={this.props.onLayout}>
        {this.props.children}
      </View>
    );
  }
